  ...
  try {
    if (req.method === 'POST') {
      ...
      const ierc20Source = fs.readFileSync(
        path.join('node_modules/@openzeppelin/contracts/token/ERC20/IERC20.sol'),
        'utf-8',
      );
      const ierc20MetadataSource = fs.readFileSync(
        path.join('node_modules/@openzeppelin/contracts/token/ERC20/extensions/IERC20Metadata.sol'),
        'utf-8',
      );
