...
import { AreonNetworks } from '@modules/blockchains/areon';

const AreonAppPage: NextPage = () => {
  const blockchain = Blockchains.Areon;
    ...
    const init = async () => {
      const reconnect = async () => {
        ...
        if (walletConnected) {
          ...
          if (network !== AreonNetworks.Mainnet && network !== AreonNetworks.Testnet) {
            showMessage(
              'Not connected to Areon',
              'Your wallet is connected to a different chain. Please switch to Areon in order to sign in to Block Fabric for Areon.',
            );
            return;
          }
