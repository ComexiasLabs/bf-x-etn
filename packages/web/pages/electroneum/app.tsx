...
import { ElectroneumNetworks } from '@modules/blockchains/electroneum';

const ElectroneumAppPage: NextPage = () => {
  const blockchain = Blockchains.Electroneum;
    ...
    const init = async () => {
      const reconnect = async () => {
        ...
        if (walletConnected) {
          ...
          if (network !== ElectroneumNetworks.Mainnet && network !== ElectroneumNetworks.Testnet) {
            showMessage(
              'Not connected to Electroneum',
              'Your wallet is connected to a different chain. Please switch to Electroneum in order to sign in to Block Fabric for Electroneum.',
            );
            return;
          }
...
export default ElectroneumAppPage;
